# Script PowerShell pour importer les ligues en production
# Utilise la commande Django existante avec la configuration de production

Write-Host "🏛️  IMPORTATION DES LIGUES D'ARBITRAGE - PRODUCTION" -ForegroundColor Cyan
...
if (-not (Test-Path "data/ligues.yaml")) {
    Write-Host "❌ Erreur: Le fichier data/ligues.yaml n'existe pas" -ForegroundColor Red
    exit 1
}

Write-Host "🔧 Configuration: PRODUCTION" -ForegroundColor Green
Write-Host "🗄️ Base de données: PostgreSQL" -ForegroundColor Green
Write-Host "📁 Fichier source: data/ligues.yaml" -ForegroundColor Green
Write-Host ""
...
python manage.py import_ligues --file=data/ligues.yaml --force
